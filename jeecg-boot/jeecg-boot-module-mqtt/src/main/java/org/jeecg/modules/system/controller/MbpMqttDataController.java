...
@Api(tags="实时数据")
@RestController
@RequestMapping("/system/mbpMqttData")
@Slf4j
public class MbpMqttDataController extends JeecgController<MbpMqttData, IMbpMqttDataService> {
	...
	private IMbpMqttDataService mbpMqttDataService;
	  ...
	  * @return
	  */
	 @ApiOperation(value="实时数据-当月数据采集量", notes="实时数据-当月数据采集量")
	 @GetMapping(value = "/monthCountData")
	 public Result<?> monthCountData(MbpMqttData mbpMqttData) {
		 QueryWrapper<MbpMqttData> queryWrapper = new QueryWrapper<>();
		 String time = new SimpleDateFormat("yyyy-MM").format(new Date());
		 queryWrapper.ge("create_time", (time + "-01 00:00:00:001"));
		 int count = mbpMqttDataService.count(queryWrapper);
	  ...
	  * @return
	  */
	 @ApiOperation(value="实时数据-当天数据采集量", notes="实时数据-当天数据采集量")
	 @GetMapping(value = "/dayCountData")
	 public Result<?> dayCountData(MbpMqttData mbpMqttData) {
		 QueryWrapper<MbpMqttData> queryWrapper = new QueryWrapper<>();
		 String time = new SimpleDateFormat("yyyy-MM-dd").format(new Date());
		 queryWrapper.ge("create_time", (time + " 00:00:00:001"));
		 int count = mbpMqttDataService.count(queryWrapper);
		 return Result.OK(count);
	 }
	  ...
	  * @return
	  */
	 //@AutoLog(value = "实时数据-查询最新十条记录")
	 @ApiOperation(value="实时数据-查询最新十条记录", notes="实时数据-查询最新十条记录")
	 @GetMapping(value = "/TopTen")
	 public Result<?> TopTen(MbpMqttData mbpMqttData) {
		 List<MbpMqttData> list = mbpMqttDataService.TopTen(mbpMqttData);
